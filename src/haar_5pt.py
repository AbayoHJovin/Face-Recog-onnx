...
def align_face_with_5_points(
    ...
    """
    M = _compute_normalization_matrix(kps_5x2, out_size=out_size)
    out_w, out_h = int(out_size[0]), int(out_size[1])
...
class Haar5ptDetector:
    ...
    def detect(self, frame_bgr: np.ndarray, max_faces: int = 1) -> List[FaceDetectionResult]:
        H, W = frame_bgr.shape[:2]
...
def main():
    ...
    while True:
        ...
        if detections:
            ...
            cv2.rectangle(visualization, (detection.x1, detection.y1), (detection.x2, detection.y2), (0, 255, 0), 2)

            for x, y in detection.kps.astype(int):
                cv2.circle(visualization, (int(x), int(y)), 3, (0, 255, 0), -1)

            cv2.putText(
                visualization,
                "OK",
                (detection.x1, max(0, detection.y1 - 8)),
        ...
        cv2.imshow("haar_5pt", visualization)
